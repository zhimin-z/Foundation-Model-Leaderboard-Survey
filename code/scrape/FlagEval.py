            ...
            try:
                for level2 in driver.find_elements(By.XPATH, '//div[@class="el-form-item__content"]')[2].find_elements(By.XPATH, './/div'):
                    ...
                    try:
                        ...
                        #         df.to_json(
                        #             f'{path_leaderboard}/ip-{prepcess_name(domain_name)}-{prepcess_name(level1_name)}-{prepcess_name(level2_name)}-{prepcess_name(level3_name)}.json', orient='records', indent=4)
                    except:
                        df = retrieve_table(driver)
                        if not df.empty:
                            df.rename(columns={'Model name': 'Model'}, inplace=True)
                            df.to_json(
                                f'{path_leaderboard}/ip-{prepcess_name(domain_name)}-{prepcess_name(level1_name)}-{prepcess_name(level2_name)}.json', orient='records', indent=4)
            except:
                df = retrieve_table(driver)
                if not df.empty:
                    df.rename(columns={'Model name': 'Model'}, inplace=True)
                    df.to_json(
                        f'{path_leaderboard}/ip-{prepcess_name(domain_name)}-{prepcess_name(level1_name)}.json', orient='records', indent=4)
